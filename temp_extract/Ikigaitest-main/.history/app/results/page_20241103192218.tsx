...
const ResultsPage = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (!storedScores || Object.keys(storedScores).length === 0) {
        console.log("No stored scores found or scores are empty");
        return;
      }
      ...
      for (const category in categoryTotals) {
        ...
        categoryTotals[category as Category] = Object.values(categoryScores).reduce((acc: number, val: number) => acc + val, 0);
      }
  ...
  if (!dominantCategory || !secondaryCategory) return <div>Loading...</div>;
