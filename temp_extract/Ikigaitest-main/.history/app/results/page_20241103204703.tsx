...
const ResultsPage = () => {
  ...
  useEffect(() => {
    // Parse stored scores and use a default empty object if it fails
    const storedScores = JSON.parse(localStorage.getItem("ikigaiScores") || "{}") || {};
    
    // Check if storedScores is empty or malformed
    if (!storedScores || Object.keys(storedScores).length === 0) {
      console.warn("No scores found, using default structure");
      return;
    }
    ...
    // Safely accumulate totals with nullish coalescing
    for (const category in categoryTotals) {
      const categoryScores = storedScores[category as Category];
      if (categoryScores) {
        categoryTotals[category as Category] = Object.values(categoryScores).reduce(
