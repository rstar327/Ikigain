...
// Ensure dynamic is imported only once
import dynamic from 'next/dynamic';
import React, { useState, useEffect, useMemo } from 'react';

// Use React.memo to prevent unnecessary re-renders of the Card component
const Card = React.memo(({ title, subtitle, value }: { title: string; subtitle: React.ReactNode; value: string }) => (
  <motion.div
...
const DynamicPieChartComponent = dynamic(() => import('./PieChartComponent'), {
  ssr: false,
  loading: () => <p>Loading...</p>,
});
...
const ResultsPage = () => {
  ...
  const subcategoryData = useMemo(() => Object.keys(scores[dominantCategory])
