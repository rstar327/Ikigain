...
import { memo } from "react"; // Import memo to avoid re-renders on static components

const useMediaQuery = dynamic(() => import('react-responsive').then(mod => mod.useMediaQuery), {
  ssr: false
}) as unknown as (query: string) => boolean;

const ProgressBar = memo(({ currentQuestion, totalQuestions }: { currentQuestion: number, totalQuestions: number }) => (
...
const PersonalityTest = () => {
  ...
  // Pre-fetch results page when close to the last question
  useEffect(() => {
