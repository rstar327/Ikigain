...
export default function Dashboard() {
  ...
  // Get user's sessions (always try to get this)
  const { data: userSessions, isLoading: sessionsLoading } = useQuery({
    queryKey: ["/api/user/test-sessions"],
    retry: false,
  });

  // Get specific session data if session parameter is provided
  const { data: specificSession } = useQuery({
    queryKey: sessionId ? ["/api/test-sessions", sessionId] : ["/api/test-sessions", "none"],
    enabled: !!sessionId,
    retry: false,
  });
  ...
  // Determine user state based on test sessions or specific session
  const getUserState = () => {
    // If there's a specific session, use that session's data
    if (sessionId && specificSession) {
      const session = specificSession as any;
      const isCompleted = session.isCompleted || session.is_completed;
      
      if (!isCompleted) {
        return "incomplete_test"; // Started but never completed
      }
      
      // Check if user has premium access - more robust checking
      const hasPremiumAccess = session.hasPremiumAccess || session.has_premium_access;
      const premiumTier = session.premiumTier || session.premium_tier;
      
      console.log('Session premium check:', { hasPremiumAccess, premiumTier, sessionId });
      
      if (hasPremiumAccess === true || (premiumTier && premiumTier.trim() !== '')) {
    ...
    // Otherwise, use user's test sessions
    if (!userSessions || !Array.isArray(userSessions) || userSessions.length === 0) {
      return "new_user"; // Never taken any test
    }
    
    const completedSessions = userSessions.filter((session: any) => session.isCompleted || session.is_completed);
    const recentSession = completedSessions[0]; // Get the most recent completed session (first in array)
    ...
    const hasPremiumAccess = recentSession?.hasPremiumAccess || recentSession?.has_premium_access;
    const premiumTier = recentSession?.premiumTier || recentSession?.premium_tier;
    
    if (hasPremiumAccess || premiumTier) {
      return "premium_user"; // Has premium access
    }
  ...
  console.log('Dashboard Debug:', {
    ...
    specificSession: specificSession ? {
      ...
      hasPremiumAccess: specificSession.hasPremiumAccess,
      premiumTier: specificSession.premiumTier
    } : null,
    userState,
    userSessions: userSessions ? userSessions.length : 'none'
  });
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl text-purple-100 mb-4">
                {t('dashboard:readyToDiscover', 'Ready to discover your life\'s purpose?')}
              </p>
              <p className="text-purple-200 text-lg mb-6">
                {t('dashboard:ikigaiJourneyStarts', 'Your Ikigai journey starts here – let\'s find what makes you come alive.')}
              </p>
              <div className="flex items-center space-x-4">
                <div className="flex items-center text-purple-100">
                  <Clock className="w-4 h-4 mr-2" />
                  <span className="text-sm">{t('dashboard:fifteenMinutes', 'Just 15 minutes to life-changing insights')}</span>
                </div>
                <div className="flex items-center text-purple-100">
                  <Star className="w-4 h-4 mr-2" />
                  <span className="text-sm">{t('dashboard:joinThousands', 'Join 10,000+ purpose seekers')}</span>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Card className="bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0 shadow-lg">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-100 text-sm">{t('dashboard:peopleDiscovered', 'People discovered their purpose')}</p>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-r from-blue-500 to-cyan-600 text-white border-0 shadow-lg">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold mb-2">87%</div>
            <p className="text-blue-100 text-sm">{t('dashboard:reportSatisfaction', 'Report increased life satisfaction')}</p>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-r from-purple-500 to-pink-600 text-white border-0 shadow-lg">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-purple-100 text-sm">{t('dashboard:averageTime', 'Average time to breakthrough')}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="bg-gradient-to-br from-blue-600 to-purple-700 text-white border-0 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div>
                <Badge className="bg-white/20 text-white border-0 mb-3">{t('dashboard:mostPopular', 'Most Popular')}</Badge>
                <CardTitle className="text-2xl font-bold text-white">{t('dashboard:fullIkigaiTest', 'Full Ikigai Test')}</CardTitle>
              </div>
  ...
  const renderBasicUserDashboard = () => {
    // Get the session data - prioritize specific session if available
    const completedSessions = (userSessions as any)?.filter?.((session: any) => session.isCompleted || session.is_completed) || [];
    const latestSession = specificSession || completedSessions[0]; // Use most recent completed session
  ...
  const renderPremiumUserDashboard = () => {
    // Get the session data - prioritize specific session if available  
    const completedSessions = (userSessions as any)?.filter?.((session: any) => session.isCompleted || session.is_completed) || [];
    const sessionData = specificSession || completedSessions[0]; // Use specific session first, then most recent
    const premiumTier = sessionData?.premiumTier || sessionData?.premium_tier || 'basic';
    const sessionIdForLinks = sessionData?.id;
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl p-8 mb-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xl text-purple-100 mb-4">
              Your {getTierDisplayName(premiumTier)} insights are ready
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="bg-gradient-to-br from-indigo-50 to-blue-100 border-indigo-200 shadow-lg">
            ...
            <CardContent>
              ...
              <Link href={`/premium-results/${specificSession?.id || latestSession?.id}`}>
          ...
          {/* Show upgrade option if not at highest tier, otherwise show quick test */}
          {premiumTier !== 'blueprint' ? (
            <Card className="bg-gradient-to-br from-purple-50 to-pink-100 border-purple-200 shadow-lg">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-bold text-purple-900">Upgrade Available</CardTitle>
                  </div>
              ...
              <CardContent>
                <p className="text-purple-800 mb-4 text-sm">
                  Unlock even more insights with a higher tier premium analysis.
                </p>
                <Link href={`/upgrade?session=${latestSession?.id}`}>
                  <Button className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 text-sm">
                    Upgrade Plan
                  </Button>
                </Link>
              </CardContent>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      <SEO 
        title="Your Ikigai Dashboard - Track Your Purpose Journey | Ikigain"
        description="Access your personalized Ikigai dashboard. Track your test progress, view results, and explore premium features to deepen your life purpose discovery."
        keywords="ikigai dashboard, personal dashboard, test progress, life purpose tracking"
        canonical="/dashboard"
      />
      <Navigation />
